...
export function MapView({
  ...
  }, [onMapReady, onMapClick, onSelectSpot]);
  ...
  useEffect(() => {
    ...
    L.tileLayer(import.meta.env.VITE_TILE_URL?.trim() || DEFAULT_TILE_URL, {
      maxZoom: 19,
  ...
  useEffect(() => {
    ...
    markers.clearLayers();
    spots.forEach((spot) => {
      const marker = L.marker([spot.lat, spot.lng], { icon: pinIcon(selectedId === spot.id), title: spot.name, keyboard: true });
